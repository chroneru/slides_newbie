...
void quiz(void) {
  ...
  printf("おうとうってなんだ？: ");
  scanf("%s",ans_u);
  if(strcmp(answer,ans_u) == 0) printf("あってるよ");
  else printf("まちがってるよ");
}
...
```bash
gcc -shared -fPIC -I/usr/include/R -o quiz_c.so quiz_c.c
```

```{r quiz,eval=FALSE}
dyn.load("./C/quiz_c.so")
.C('quiz')
#> list()
#> おうとうってなんだ？: まちがってるよ
dyn.unload("quiz_c.so")
```
...
* 関数の戻り値は`void`にする。
* 引数はポインタを使う。
* R系のヘッダファイルを読み込む場合，gccで共有ライブラリを作る
* Rのバッチコマンドもあるけど使いづらい
